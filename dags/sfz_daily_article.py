...
def sfz_daily_article():
    ...
    # A single configuration read creates stable, separate nodes in the graph.
    sites = active_sites()
    ...
    for site in sites:
        ...
        article = generate_article.override(
            task_id=f"generate_article_{slug}",
            task_display_name=f"Generate article: {title}",
        )(site)
        save_run_receipt.override(
            task_id=f"save_run_receipt_{slug}",
            task_display_name=f"Save receipt: {title}",
        )(article)
