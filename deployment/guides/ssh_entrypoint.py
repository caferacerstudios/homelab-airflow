#!/usr/bin/env python3
"""Only explicit, encoded site checks and guides refreshes are allowed."""
import base64
import binascii
import json
import os
...
def command_arguments(command: str) -> list[str]:
    ...
    try:
        ...
        raise ValueError('Invalid encoded guides request') from exc
    ...
    if not isinstance(request, dict) or set(request) != expected or not isinstance(request['site'], dict):
        raise ValueError('Unexpected guides request fields')
...
def main():
    try:
        ...
        return 2
    return subprocess.run([sys.executable, '-B', str(HERE / 'refresh_guides.py'), *args], check=False).returncode
