#!/usr/bin/env python3
"""Install the dedicated guides SSH connection; never start a source refresh."""
...
print('GUIDES_ACTIVE_SITES=' + json.dumps(sites, separators=(',', ':')))
"""
    ...
    lines = [line.removeprefix('GUIDES_ACTIVE_SITES=') for line in result.stdout.splitlines()
             if line.startswith('GUIDES_ACTIVE_SITES=')]
    if len(lines) != 1:
...
def install_key() -> str:
    ...
    private = secrets / "sfz_guides_ed25519"
    ...
    if not private.exists() and not private.is_symlink():
        ...
        run([
            ...
            "sfz-guides-airflow", "-f", str(private),
    ...
    if public.exists() or public.is_symlink():
        ...
        public.write_text(public_key + " sfz-guides-airflow\n")
    public.chmod(0o644)
    ...
    line = (
        ...
        f"{public_key} sfz-guides-airflow"
    )
...
try:
    ...
    if payload["conn_id"] != "sfz_guides_host":
...
bag = BundleDagBag(dag_folder='/opt/airflow/dags/sfz_game_guides.py', bundle_path=Path('/opt/airflow/dags'))
assert not bag.import_errors, bag.import_errors
dag = bag.dags.get('sfz_game_guides')
assert dag is not None, 'Guides refresh DAG was not found'
assert {task.task_id for task in dag.tasks} == {
    name + '_' + site['slug'] for site in sites
    for name in ('refresh_guides', 'save_run_receipt')
}, 'DAG active-site tasks differ from installer selections'
hook = SSHHook(ssh_conn_id='sfz_guides_host', cmd_timeout=120)
with hook.get_conn() as client:
    for site in sites:
        ...
        if status:
            ...
            raise SystemExit('Source-free guides SSH check failed for ' + site['slug'])
        print(output.decode('utf-8', errors='replace').strip())
print('Guides DAG import and source-free SSH checks passed. No source collection was started.')
"""
...
def main() -> int:
    ...
    if os.getuid() == 0 or account.pw_name != "laurawkr" or PROJECT != EXPECTED_PROJECT:
        raise RuntimeError("Run this as laurawkr from /home/laurawkr/homelab-airflow/deployment/guides/install.py; do not run the installer with sudo")
