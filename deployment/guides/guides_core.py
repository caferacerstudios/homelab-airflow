...
def requires_event_evidence(field: str, row: dict) -> bool:
    # General venue guidance and recurring viewing/tailgate options are useful
    # without an announcement naming this exact game. Label them on export.
    return (field in ('localTv', 'streams', 'national')
...
def make_records(draft: dict, sources: list[dict], game: dict, site: dict, now: datetime) -> tuple[dict, dict, dict]:
    ...
    for field in ('alerts', 'transportation', 'parking', 'timeline', 'tailgates', 'watchParties', 'stadiumTips'):
        ...
        for index, row in enumerate(values):
            ...
            if row['scope'] == 'standing-policy':
                # Use fields the existing website already renders. Keep the
                # source link and reported details; don't imply a dated event.
                labels = {'alerts': ('title', 'General guidance: '),
                          'timeline': ('time', 'Typical: '),
                          'tailgates': ('name', 'Recurring option: '),
                          'watchParties': ('name', 'Viewing option: ')}
                if field in labels:
                    label_field, prefix = labels[field]
                    if not clean[label_field].startswith(prefix):
                        clean[label_field] = prefix + clean[label_field]
                if field == 'alerts':
                    clean['severity'] = 'info'
            if field == 'tailgates':
...
def generate(directory: Path, key: str, config: dict, game: dict, site: dict, now: datetime,
    ...
    city = site['city']
    research_prompt = f'''Today is {iso(now)}. Act as a local service reporter researching a thorough game-day and viewing guide for one NFL game. Give the writer enough sourced, practical detail for a fan to plan the trip, prepare for entry, choose a pregame activity or a place to watch, and get home afterward. Useful linked options belong in this guide even when a venue has not posted a separate announcement for this matchup. Distinguish what is confirmed for the game from ordinary or recurring arrangements, without turning the guide into a list of caveats.
...
3. Research named tailgates and places to watch. Look for current venue/operator pages, team-supporter clubs, recurring football-viewing programs and tailgate organizers, as well as dated event announcements. A current primary page showing that a named venue screens sports or hosts a recurring football gathering is enough to include it as a viewing or recurring option; do not require a separate dated announcement for every bar. One direct supporting primary page per listing is sufficient. Look across the home fan region, including useful neighborhoods or nearby towns, rather than only downtown. Preserve venue/address or area, ordinary hours clearly described as usual, game sound/screens, age/admission/reservation requirements and non-price features when documented. Record whether the page establishes a confirmed party for this game, a recurring program or simply a useful sports-viewing venue. Do not turn normal hours into a confirmed event start time or an old one-off party into a current recurring program. Leave unannounced details unknown while retaining the useful venue option.
...
EVIDENCE RULES
For each finding, preserve its direct supporting page and distinguish: (a) confirmed for this exact event, with the full calendar date/year, matchup, NFL venue and relevant access conditions checked; (b) currently applicable general guidance, a typical schedule, a recurring activity or a venue viewing option, with no claim of a special game-day arrangement; or (c) an unresolved question. A party described as confirmed for this game must refer to the correct NFL game/date, but its physical location is the organizer's actual venue, not the NFL stadium. General venue options do not need matchup-specific confirmation. Page retrieval time is not the announcement date. Generic URLs may contain an old game; conflicting opponents or years invalidate event confirmation. A copied EVENT date is not supporting evidence.

General constraints, published typical arrival milestones, recurring tailgates and sports-viewing venues can use their current supporting general pages. Separate these from confirmed closures, special gate times and one-off parties. Special transport and broadcast claims need exact-event evidence. Sounder service must be explicitly confirmed by Sound Transit for this event/date, including the return arrangement. An absent listing is unknown, never proof that service is canceled or unavailable. Ordinary applicable transit, parking, stadium-entry and venue information can support a useful guide when event-specific announcements are sparse. Do not present an unconfirmed special event as a confirmed or recurring arrangement.
...
Provide a detailed, source-cited reporting brief organized into: practical overview; transportation and return trip; parking; entry/accessibility; general constraints and dated alerts; typical and confirmed activities; tailgates; home-market viewing venues and watch parties; television/streaming; and specific unknowns. For each usable finding include the actionable detail, direct citation, evidence scope/date and any limit on what the page establishes. Preserve distinct useful options and access restrictions for the writer. Keep general guidance separate from dated extras; if there are no confirmed parties or ceremonies, still supply useful travel, parking, entry and venue options. Do not manufacture a number of sources or items to meet the coverage targets.'''
...
DEPTH AND PURPOSE
Aim for approximately 900–1,300 useful words across the reader-facing sections when the evidence supports that depth. This is an editorial target, not a quota. A well-reported game may support roughly 12–20 distinct practical items across the guide plus viewing information. Preserve the real options and useful details; never pad, repeat a fact as several items, or invent information to reach a count. Sparse dated announcements should reduce confirmed special-event claims, not erase useful general transportation, parking, entry, recurring tailgates or sports-viewing venues. Readers can follow the supplied source links for current details; they do not need every venue independently confirmed for this matchup.

SECTION EXPECTATIONS
- summary: aim for 90–150 words. Lead with the most important supported arrival choice, constraint or useful activity, and explain what it means for the fan. Give a useful overview of the trip, entry preparation and local viewing options rather than hype or a generic reminder to check websites. If the research establishes applicable general guidance or useful venue options, write a substantive standing-policy summary with eventDate null; do not return null merely because ceremonies, watch parties or broadcasts are unannounced. An event-specific summary must have evidence for its dated claims. Keep general venue recommendations distinct from promises of a confirmed party. Use null only when the research truly cannot support a useful summary.
...
- alerts: include useful supported constraints as well as dated changes, with an accurate practical consequence and proportionate severity. General rules use standing-policy scope and should read as general guidance; confirmed closures or special arrangements use event-specific scope. Unknown information and an absent listing do not prove a closure, cancellation or lack of service.
- timeline: retain published typical milestones using standing-policy scope, as well as confirmed milestones using event-specific scope. Describe ordinary schedules explicitly as usual or typical and prefer the source's relative wording, such as "Two hours before kickoff," over a calculated game-specific clock time. Order milestones chronologically and deduplicate them. Preserve differences between gates, registration, separate events and ordinary game entry; label the applicable local timezone on clock times. No guessed arrival times or invented milestones.
- tailgates: include named recurring programs and useful current tailgate options without demanding a dated announcement for each game. Use standing-policy scope for a documented recurring arrangement and event-specific scope for a confirmed party for this matchup. Clearly distinguish official activities from independent organizers. Preserve the sourced location and admission/registration/age conditions. Describe published normal hours as usual; if no applicable start time is documented, use "Check venue for this game" in startTime. Do not infer that a game ticket grants entry to a separate activity or turn an old one-off event into a recurring program.
- watchParties: write a useful local listings guide with named venues, neighborhood/town or a sourced address, and documented screens/game sound, admission/age/reservation details. A current venue/operator page showing sports viewing, a team-supporter club or a recurring football program is enough to include a general viewing option with standing-policy scope. One direct supporting page per listing is sufficient; include several distinct options when the research supplies them. Only label a party confirmed for this matchup when its event-specific evidence supports that claim. Normal hours must be described as usual; if no applicable start time is documented, use "Check venue for this game" in required startTime. Keep known venue details even when this game's timing is unannounced. Non-price features can appear in specials only when supported. Do not invent a venue, its location, a promotion or a party.
...
Every fact object needs one to four DISTINCT IDs copied exactly from SOURCE IDS, with the direct supporting page first. One directly relevant source is enough for an individual listing. The producer supplies the reader's link from the source ID, so do not write URLs in prose. Web-search markers are not source IDs. No empty sourceIds, invented IDs or additional JSON fields. For scope event-specific, the reporting must establish the correct full calendar date/year and matchup, without contradicting the NFL venue or other identity in EVENT, and eventDate must equal EVENT.date. A home-market watch party's physical venue is its actual local location; it does not need to be at the NFL stadium. Merely copying the date is not evidence. Use standing-policy with eventDate null for supported current general guidance, typical schedules, recurring activities and venue viewing options. These do not need a dated matchup page. The producer labels general alerts, typical timeline entries, recurring tailgates and viewing options automatically; do not repeat those labels in your titles or add the same confirmation disclaimer to every description.

Every published Sounder transportation fact, localTv, streams and national still requires exact-event evidence. For alerts, timeline, tailgates and watchParties, choose the scope that matches what the source actually says; a genuine general option is useful without a dated announcement. Omit unsupported details from an otherwise useful listing instead of omitting the entire venue, and do not present speculative special arrangements as established general ones. Mention an important remaining uncertainty briefly where it helps the reader decide or in unknowns, without repetitive caveats. Missing optional details remain null or an empty list. Do not turn absence of a listing into cancellation or unavailability. Do not invent gates/hours, walk times, access rights, TV stations, subscriptions, restrictions, weather, ticket listings, monetary prices or promotions.
