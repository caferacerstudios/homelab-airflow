...
STATE = Path("/var/lib/sfz-news")
CONNECTION_ID = "sfz_news_host"
...
def install_key() -> str:
    ...
    private = secrets / "sfz_news_ed25519"
    ...
    if not private.exists() and not private.is_symlink():
        ...
        run([
            ...
            "sfz-news-airflow", "-f", str(private),
    ...
    if public.exists() or public.is_symlink():
        ...
        public.write_text(public_key + " sfz-news-airflow\n")
    public.chmod(0o644)
...
try:
    ...
    if payload["conn_id"] != "sfz_news_host":
...
bag = BundleDagBag(dag_folder="/opt/airflow/dags/sfz_daily_article.py", bundle_path=Path("/opt/airflow/dags"))
assert not bag.import_errors, bag.import_errors
assert "sfz_daily_article" in bag.dags, "News refresh DAG was not found"
hook = SSHHook(ssh_conn_id="sfz_news_host", cmd_timeout=120)
...
def main() -> int:
    ...
    if os.getuid() == 0 or account.pw_name != "laurawkr" or PROJECT != EXPECTED_PROJECT:
        raise RuntimeError("Run this as laurawkr from /home/laurawkr/homelab-airflow/deployment/news/install.py; do not run the installer with sudo")
    check_owned(PROJECT, directory=True)
    for name in ("refresh_news.py", "news_core.py", "ssh_entrypoint.py"):
        check_owned(HERE / name)
    check_owned(PROJECT / "dags", directory=True)
    (PROJECT / "dags").chmod(0o755)
    for name in ("sfz_daily_article.py", "sfz_news_hook.py"):
        check_owned(PROJECT / "dags" / name)
        (PROJECT / "dags" / name).chmod(0o644)
    ensure_state_directory()
    for folder in ("photos", "assets", "days", "releases"):
        ensure_user_directory(STATE / folder, 0o755)
    config = STATE / "config.json"
    if not config.exists():
        config.write_text(json.dumps({"model": "gpt-5.4-mini"}, indent=2) + "\n")
        config.chmod(0o644)
    run([sys.executable, str(HERE / "refresh_news.py"), "--check"])
