...
STATE = Path("/var/lib/sfz-nfl")
CONNECTION_ID = "sfz_nfl_host"
...
def install_key() -> str:
    ...
    private = secrets / "sfz_nfl_ed25519"
    ...
    if public.exists() or public.is_symlink():
        ...
        public.write_text(public_key + " sfz-nfl-airflow\n")
    public.chmod(0o644)
...
try:
    ...
    if payload["conn_id"] != "sfz_nfl_host":
...
def verify_airflow() -> None:
    run([
        ...
        "pools", "set", "balldontlie_api", "1", "Serialize Seahawks NFL API refreshes",
    ], cwd=PROJECT)
    code = r'''
from pathlib import Path
...
bag = BundleDagBag(dag_folder="/opt/airflow/dags/sfz_nfl_refresh.py", bundle_path=Path("/opt/airflow/dags"))
assert not bag.import_errors, bag.import_errors
assert "sfz_nfl_refresh" in bag.dags, "NFL refresh DAG was not found"
hook = SSHHook(ssh_conn_id="sfz_nfl_host", cmd_timeout=120)
...
def main() -> int:
    ...
    if os.getuid() == 0 or account.pw_name != "laurawkr" or PROJECT != EXPECTED_PROJECT:
        raise RuntimeError("Run this as laurawkr from /home/laurawkr/homelab-airflow/deployment/nfl/install.py; do not run the installer with sudo")
    check_owned(PROJECT, directory=True)
    for name in ("refresh_nfl.py", "ssh_entrypoint.py"):
        check_owned(HERE / name)
    for name in ("sfz_nfl_refresh.py", "sfz_nfl_hook.py"):
        check_owned(PROJECT / "dags" / name)
    ensure_state_directory()
    run([sys.executable, str(HERE / "refresh_nfl.py"), "--check"])
