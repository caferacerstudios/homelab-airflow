...
import sfz_roster_hook as hook
...
class RosterHookTests(unittest.TestCase):
    def setUp(self):
        ...
        self.receipt = {"schema_version": 1, "pipeline": "roster", "runId": "manual__roster-test", "team": "broncos",
                        "updatedAt": "2026-09-01T12:00:00Z", "requestCount": 3,
    ...
    def test_request_roundtrip_preserves_configuration(self):
        ...
        self.assertNotIn("roster_snapshot_dir", value["site"])
    ...
    def test_receipts_reject_missing_mixed_foreign_and_invalid_files(self):
        ...
        variants = [dict(valid, pipeline="nfl"), dict(valid, team="seahawks"), dict(valid, runId="another"),
                    dict(valid, files={"roster.json": "a" * 64}),
                    dict(valid, files={**valid["files"], "players.json": "a" * 64}),
                    dict(valid, files={**valid["files"], "injuries.json": "not-a-hash"}),
                    dict(valid, requestCount=True), dict(valid, requestCount=-1),
    ...
    def test_artifacts_separate_teams_for_the_same_run(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for slug in ("seahawks", "broncos"):
                ...
                self.assertEqual(path.relative_to(tmp).parts[:2], (slug, "roster"))
